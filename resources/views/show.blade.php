    ...
    <div class="container product-container mx-auto mt-16 sm:px-12 sm:text-lg">
        <div class="grid grid-cols-1 sm:grid-cols-2 text-center">
            <div class="p-2 half-width flex flex-col">
                <div class="w-3/4 mx-auto border-2 rounded-lg">
                    <img class="w-48 sm:w-64 m-auto" src="/images/{{ $product['image'] }}" alt="productimg" />
                </div>
                <div class="flex justify-evenly w-3/4 mx-auto mt-12">
                    <img class="w-24 border-2 rounded-md rounded-lg p-1 cursor-pointer hover:bg-orange" src="/images/{{ $product['image'] }}" alt="productimg" />
                    <img class="w-24 border-2 rounded-md rounded-lg p-1 cursor-pointer hover:bg-orange" src="/images/{{ $product['image'] }}" alt="productimg" />
                    <img class="w-24 border-2 rounded-md rounded-lg p-1 cursor-pointer hover:bg-orange" src="/images/{{ $product['image'] }}" alt="productimg" />
                </div>
            </div>
            <div class="p-2 flex flex-col justify-between font-semibold">
                ...
                <button class="add-to-cart-btn mt-10 text-white rounded-lg flex mx-auto w-1/2 h-10">
                    <svg class="w-1/6 p-2 bg-gray-800" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 3h2l.4 2M7 13h10l4-8H5.4M7 13L5.4 5M7 13l-2.293 2.293c-.63.63-.184 1.707.707 1.707H17m0 0a2 2 0 100 4 2 2 0 000-4zm-8 2a2 2 0 11-4 0 2 2 0 014 0z" />
                    </svg>
                    <a href="{{ route('product.addtocart', ['id' => $product['id']]) }}" class="self-center mx-auto"><span class="py-2 px-1 font-bold text-xs">ADD TO CART</span></a>
                </button>
            </div>
        </div>
    </div>
